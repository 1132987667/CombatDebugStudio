...
改动以下位置时，先对号入座检查这四类模式（本仓库已各踩过一次）：

1. **枚举/结构新增 → 消费端遗漏。** 新增原子效果类型、事件码、状态字段时，
   生产侧（effect handler）之外必须 grep 它的每个消费点（投影 mapper、
   effectLines 摘要、UI 管道）。提取逻辑只扫顶层容器时，追问一句：
   "这个语义有没有嵌套在子结构 params 里的变体？"（AURA 的 modifiers 就是这样漏的。）

2. **清理与重建必须成对。** 凡是写了 off/disconnect/clear 的地方，必须指出
   对称的 on/reconnect 路径由谁、在什么时机调用；初始化逻辑若写在单例
   setup 体里（只跑一次），要提取成幂等函数暴露重挂入口。组件卸载会触发
   清理、而持有者是 app 级单例时，dev HMR 必冻结——生产不可达不等于不用修。

3. **缺省回退禁止 `||`，一律 `??`；边界值语义要枚举完整。** 数值配置里的
   0 是合法业务值（0 回合），`x || 默认` 会静默吞掉。定义数值语义时把
   全值域说清（-1 永久、正数回合、0 = ?），并检查每个比较条件
   （> 0 / <= 0）在边界值上的行为，不能只覆盖"正常流程"的正数。

4. **跨层编码约定要单源。** 领域与 UI 各自定义"永久/无效"哨兵值时
   （领域 -1 = 永久，UI 曾约定 <= 0 = 永久），漂移只是时间问题。
   哨兵值判定写一处（共享类型/工具函数），显示层引用它，不各自内联
   `<= 0` / `=== 0` 判断。
...
斗战西游是单机游戏，不需要任何日活/KPI/留存指标相关设计，让玩家玩的愉快，有爽感，可以有轻度的肝度，持续让玩家获得"变强 → 验证 → 碾压 → 再变强"的正反馈循环，
