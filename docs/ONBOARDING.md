...
- **根组件**: `src/presentation/views/BattleArena.vue` — 主布局（参战配置 + 战场 + 调试面板）
...
│   │   └── constants/      #   常量
│   └── presentation/       # 表现层
│       ├── views/          #   页面级组件（BattleArena, BattleField...）
│       ├── components/     #   可复用组件
...
- 树状调试日志（`TraceLogCollector`）跟踪每回合的技能执行链路
...
`BattleArena.vue` 提供多个调试入口：

| 对话框 | 功能 |
|--------|------|
| **数据快照** | 查看 BattleState 完整 JSON |
| **战斗记录** | 录制/保存/回放/删除战斗 |
| **调试日志** | TreeView 展示 TraceLogCollector 收集的技能执行链路 |
| **调试面板** | 断点暂停（BATTLE_START / TURN_START / TURN_END） |
| **图鉴** | 浏览可用技能和 Buff 配置 |
| **战斗规则** | 切换速度优先/暴击/闪避、调速 |
| **场景管理** | 加载/保存战斗场景配置 |
| **角色编辑** | 查看/编辑角色属性，手动添加 Buff |

### 快捷键

控制栏支持战斗开始/暂停/单步执行/自动战斗/重置。
