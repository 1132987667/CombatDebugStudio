...
export const useBattleStore = defineStore('battle', () => {
  ...
  const battleManager = shallowRef<BattleManager | null>(null)

  // 🔹 3. 事件订阅管理器（防止内存泄漏）
  const cleanupFns: Array<() => void> = []

  const subscribeEvent = <T extends keyof typeof EventHandlers>(event: T, handler: EventHandlers[T]) => {
    if (!battleManager.value) return
    const manager = battleManager.value
    // 兼容原项目 manager.on 封装
    manager.on(event as any, handler as any)
    cleanupFns.push(() => manager.off(event as any))
  }

  interface EventHandlers {
    [BattleSystemEvent.BATTLE_LOG]: (data: any) => void
    [BattleSystemEvent.BATTLE_END]: (data: any) => void
    [BattleSystemEvent.TURN_START]: (data: any) => void
    [BattleSystemEvent.TURN_END]: (data: any) => void
    [BattleSystemEvent.DAMAGE_ANIMATION]: (data: any) => void
    [BattleSystemEvent.MISS_ANIMATION]: (data: any) => void
    [BattleSystemEvent.BUFF_EFFECT]: (data: any) => void
    [BattleSystemEvent.SKILL_EFFECT]: (data: any) => void
  }

  const bindEventListeners = () => {
    if (!battleManager.value) return
    subscribeEvent(BattleSystemEvent.BATTLE_LOG, handleBattleLogEvent)
    subscribeEvent(BattleSystemEvent.BATTLE_END, handleBattleEndEvent)
    subscribeEvent(BattleSystemEvent.TURN_START, handleTurnStartEvent)
    subscribeEvent(BattleSystemEvent.TURN_END, handleTurnEndEvent)
    subscribeEvent(BattleSystemEvent.DAMAGE_ANIMATION, handleDamageAnimationEvent)
    subscribeEvent(BattleSystemEvent.MISS_ANIMATION, handleMissAnimationEvent)
    subscribeEvent(BattleSystemEvent.BUFF_EFFECT, handleBuffEffectEvent)
    subscribeEvent(BattleSystemEvent.SKILL_EFFECT, handleSkillEffectEvent)
  }
  ...
  return {
    // State
    rules, currentActorId, loading, error, autoPlayMode, isBattleActive,
    animationState, currentBattleId, turnOrder, battleSpeed, filters,
    processedActionIds, battleManager,
    // Actions
    ...
    syncBattleLogs, getSkillName, shouldDisplayLog,
  }
})