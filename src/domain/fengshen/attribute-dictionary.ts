...
export const ATTRIBUTE_DICTIONARY: AttributeDictEntry[] = [
  // ===== 运行时（4）：血条/法力条/护盾条 + 账号级 meta，不进战斗数值体系 =====
  archived('currentHealth', '当前气血', 'L1', '运行时', '运行时状态：单位当前生命值'),
  archived('currentEnergy', '当前法力', 'L1', '运行时', '运行时状态：单位当前法力'),
  ...
  core('hitRate', '命中率', 'L2', '生存对抗'),
  core('dodgeRate', '闪避率', 'L2', '生存对抗'),
  ...
  core('energyInit', '初始法力', 'L2', '机制节奏'),
  core('energyGainEfficiency', '法力获取效率', 'L2', '机制节奏'),
  ...
  archived('maxEnergy', '最大法力', 'L1', '技能机制', '用户剔除：法力上限固定，引擎强引用(82处)，仅移出数值视图'),

  // ===== 归档：状态修正 / 条件增伤 =====
  archived('controlDurationReduction', '受控时间减免', 'L2', '状态修正', '用户剔除：控制对抗细节，扩展预留'),
  // critDamageTaken（受到暴击伤害）已彻底废弃（2026-09-22）：与暴伤减免重叠且引擎读取语义相反，
  // 定义移入 configs/expired/attributes-expired.json，字典与 JSON 同步物理删除（同 damageTakenIncrease 先例）
