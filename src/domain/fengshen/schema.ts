...
export const TABLE_SCHEMAS: Record<FengshenTableName, TableSchema> = {
  ...
  skills: {
    ...
    fields: [
      ...
      { key: 'maxUses', label: '最大使用次数', type: 'number', min: 0, max: 99,
        description: '单场战斗限用次数；缺省 = 不限', column: { format: 'number' } },
    ],
    uniqueFields: ['name'],
    filters: [
  ...
  buffs: {
    ...
    fields: [
      ...
      { key: 'tags', label: '标签', type: 'array',
        description: '机制标签（burn/taunt/guardian/no_shield 等），供引擎按 tag 拦截/联动',
        arrayTemplate: ['burn'] },
      { key: 'dispellable', label: '可驱散', type: 'boolean',
        description: '缺省 = 按引擎默认规则；true 强制可驱散，false 强制不可驱散' },
      { key: 'description', label: '描述', type: 'text', searchable: true },
    ],
    uniqueFields: ['name'],
    filters: [
  ...
  enemies: {
    ...
    fields: [
      ...
      { key: 'faction', label: '阵营', type: 'select', enum: ['metal', 'wood', 'water', 'fire', 'earth', 'neutral'], searchable: true,
        valueLabel: { metal: '金', wood: '木', water: '水', fire: '火', earth: '土', neutral: '中立' } },
      { key: 'exp', label: '经验奖励', type: 'array',
        description: '经验区间 [min, max]，战斗结算时随机取值',
        arrayTemplate: [10, 20] },
      { key: 'money', label: '金钱奖励', type: 'array',
        description: '金钱区间 [min, max]，战斗结算时随机取值',
        arrayTemplate: [5, 15] },
      { key: 'affixPool', label: '词条池', type: 'object',
        description: '战斗内词缀抽取参数',
        objectTemplate: { buffTier: 0, count: 0 } },
    ],
    uniqueFields: ['name'],
    filters: [
      {
  ...
  equipment: {
    table: 'equipment',
    label: '装备',
    columns: ['name', 'slot', 'tier', 'rarity', 'requiredLevel', 'cost'],
    fields: [
      { key: 'name', label: '名称', type: 'text', required: true },
      ...
      { key: 'craftable', label: '可打造', type: 'boolean',
        description: 'true 的可打造装备在物品经济「装备」表按 craftable 筛选即得原「装备详情」清单' },
  ...
  equipment_affixes: {
    ...
    fields: [
      ...
      { key: 'applicableSlots', label: '适用部位', type: 'array', required: true, searchable: true,
        ...
        arrayTemplate: ['weapon:轻型', 'glove:护手'] },
