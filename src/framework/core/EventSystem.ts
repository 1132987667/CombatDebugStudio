...
export class EventSystem implements IEventSystem {
   ...
   * 注册事件监听器
   */
  public on(event: string, listener: EventListener, options: { priority?: number; async?: boolean } = {}): void {
   ...
   * 一次性事件监听器
   */
  public once(event: string, listener: EventListener, options: { priority?: number; async?: boolean } = {}): void {
    const { priority = 0, async = false } = options;
   ...
   * 批量注册事件监听器
   */
  public registerEventMap(eventMap: Record<string, EventListener | EventListener[]>): void {
    ...
    this.logger.debug('Event map registered', { eventCount: Object.keys(eventMap).length });
  }
   ...
   * 触发事件并等待所有监听器完成
   */
  public async emitAsync(event: string, data?: any, source?: string): Promise<void> {
    if (!this.isEnabled) {
      return;
    }
  ...
  public getEventStats(): EventStats {
    const stats: EventStats = {
      ...
      eventsByType: {},
      lastEventTime: this.eventHistory.length > 0 ? this.eventHistory[this.eventHistory.length - 1].timestamp : 0
    };

    for (const eventData of this.eventHistory) {
      stats.eventsByType[eventData.event] = (stats.eventsByType[eventData.event] || 0) + 1;
    }
   ...
   * 执行事件监听器
   */
  private executeListeners(listeners: EventListenerWrapper[], eventData: EventData): void {
   ...
   * 执行异步事件监听器
   */
  private async executeAsyncListeners(listeners: EventListenerWrapper[], eventData: EventData): Promise<void> {
