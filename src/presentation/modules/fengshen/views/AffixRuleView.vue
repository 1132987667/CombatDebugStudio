<template>
  <div class="fs-list-view">
    ...
    <div v-if="driftFromConfigs" class="fs-ov-drift" role="status">
      <span class="fs-ov-drift-text">当前存档的投放规则与 <code>configs/equipment/affix-rule.json</code>
        定稿不一致（子类型、核心系数等可能仍是旧值）。</span>
    ...
    <section v-if="activeTab === 'overview'" class="fs-exp-panel" role="tabpanel">
      ...
      <div class="fs-ov-params">
        <div class="fs-ov-params-head">
          ...
          <Button size="small" :disabled="!overview" title="缓存当前推算参数供并排对比（最多 3 份）" @click="saveOvSnapshot">存为对比快照（{{
            ovSnapshots.length }}/3）</Button>
        </div>
      ...
      <div v-if="overview" class="fs-ov-grid">
        ...
        <div class="fs-ov-card fs-ov-main">
          <div class="fs-ov-subject">
            ...
            <span class="fs-ov-subject-text">{{ slotGroupLabel(ovSlot) }} · {{
              (cfg.sub_type_groups[ovSlot]?.sub_types.find(s => s.id === ovSubType))?.name ?? ovSubType}} · Lv.{{
                ovLevel }} · {{ tierLabel(ovTier) }} · 附加 {{ ovQuality }} 条</span>
          </div>
          ...
          <div class="fs-ov-section">
            ...
            <table class="fs-table fs-ov-table">
              <thead>
                <tr>
                  <th>属性</th>
                  <th class="fs-td-right">可获得区间</th>
                  <th class="fs-td-center">来源</th>
                </tr>
              </thead>
              <tbody>
                <tr v-if="overview.core">
                  ...
                  <td class="fs-td-right fs-cell-num">
                    <CalcBreakdown :value="fmtRange(overview.core)" :steps="calcSteps(overview.core, '未配置核心属性系数')"
                      :unit="rangeUnit(overview.core)" />
                  </td>
                  <td class="fs-td-center"><span class="fs-ov-src">装备公式</span></td>
                </tr>
                <tr v-else>
                  <td colspan="3" class="fs-cell-dim">未配置核心属性系数</td>
                </tr>
              </tbody>
            </table>
          </div>
            ...
            <table class="fs-table fs-ov-table">
              <thead>
                <tr>
                  <th>词条</th>
                  <th>内容</th>
                  <th class="fs-td-right">数值区间</th>
                </tr>
              </thead>
              <tbody>
                <tr>
                  <td>第 1 条 · 固定</td>
                  <td v-if="overview.mainFixed" class="fs-td-strong">{{ attrNameByCode(overview.mainFixed.attribute) }}
                  </td>
                  <td v-else class="fs-cell-dim">未配置</td>
                  <td class="fs-td-right fs-cell-num">
                    <CalcBreakdown :value="fmtRange(overview.mainFixed)"
                      :steps="calcSteps(overview.mainFixed, '子类型主要属性第 1 条未配置')" :unit="rangeUnit(overview.mainFixed)" />
                  </td>
                </tr>
                <tr class="fs-ov-clickable" :class="{ 'is-active': ovPool === 'main' }" tabindex="0" role="button"
                  aria-label="展开主要属性随机池" @click="ovPool = 'main'" @keydown.enter.prevent="ovPool = 'main'"
                  @keydown.space.prevent="ovPool = 'main'">
                  <td>第 2 条 · 随机</td>
                  <td>
            ...
            <table class="fs-table fs-ov-table">
              <thead>
                <tr>
                  ...
                  <th>名称</th>
                  <th>属性池</th>
                  <th class="fs-td-center">本品质</th>
                </tr>
              </thead>
              <tbody>
                <tr v-for="row in overview.affixRows" :key="row.row" class="fs-ov-clickable"
                  :class="{ 'is-active': ovPool === `row-${row.row}`, 'is-muted': !row.included }" tabindex="0"
                  role="button" :aria-label="`展开第 ${row.row} 行 ${row.name} 词条池`"
                  :aria-pressed="ovPool === `row-${row.row}`" @click="ovPool = `row-${row.row}`"
                  @keydown.enter.prevent="ovPool = `row-${row.row}`" @keydown.space.prevent="ovPool = `row-${row.row}`">
            ...
            <table class="fs-table fs-ov-table">
              ...
              <tbody>
                <tr v-for="c in ovPoolDetail.candidates" :key="c.attribute" :class="{ 'is-none': c.source === 'none' }">
                  ...
                  <td class="fs-td-right fs-cell-num">
                    <CalcBreakdown :value="fmtRange(c)" :steps="c.calc" :unit="rangeUnit(c)" />
                  </td>
                  <td class="fs-td-center"><span class="fs-ov-src">{{ c.source === 'formula' ? '装备公式' : c.source ===
                    'curve' ? '曲线' : '缺曲线' }}</span></td>
                </tr>
                <tr v-if="!ovPoolDetail.candidates.length">
                  <td colspan="3" class="fs-cell-dim">该池下无可投放属性</td>
                </tr>
              </tbody>
            </table>
          </template>
      ...
      <div v-if="ovSnapshots.length" class="fs-ov-snapshots">
        ...
        <div class="fs-ov-snapshot-grid">
          <div v-for="(item, i) in snapshotOverviews" :key="i" class="fs-ov-card">
            <div class="fs-ov-subject">
              <span class="fs-affix-tag-sm" :class="snapshotSide(item.snap) === 'ATK' ? 'side-atk' : 'side-def'">{{
                snapshotSide(item.snap) }}</span>
              ...
              <tbody>
                <tr>
                  ...
                  <td class="fs-td-right fs-cell-num">
                    ...
                    <span
                      v-if="snapshotDelta(item.overview.core?.max ?? null, snapshotOverviews[0]?.overview.core?.max ?? null)"
                      class="fs-ov-snap-delta"
                      :class="{ neg: (item.overview.core?.max ?? 0) < (snapshotOverviews[0]?.overview.core?.max ?? 0) }">
                      vs 首份 {{ snapshotDelta(item.overview.core?.max ?? null, snapshotOverviews[0]?.overview.core?.max
                        ??
                        null) }}
                    </span>
                  </td>
                ...
                <tr>
                  ...
                  <td class="fs-td-right fs-cell-num">
                    ...
                    <span
                      v-if="snapshotDelta(item.overview.mainFixed?.max ?? null, snapshotOverviews[0]?.overview.mainFixed?.max ?? null)"
                      class="fs-ov-snap-delta"
                      :class="{ neg: (item.overview.mainFixed?.max ?? 0) < (snapshotOverviews[0]?.overview.mainFixed?.max ?? 0) }">
                      vs 首份 {{ snapshotDelta(item.overview.mainFixed?.max ?? null,
                        snapshotOverviews[0]?.overview.mainFixed?.max ?? null) }}
                    </span>
                  </td>
                </tr>
                <tr v-for="row in item.overview.affixRows" :key="row.row" :class="{ 'is-none': !row.included }">
    ...
    <section v-else-if="activeTab === 'pet_mount_overview'" class="fs-exp-panel" role="tabpanel">
      <div class="fs-ov-params">
        <div class="fs-ov-params-head">
          <span class="fs-block-title">推算参数</span>
          <span class="fs-form-hint">按当前投放规则反推一只宠物/一匹坐骑可产出的全部属性区间。数值为规则推算，非引擎结算；个体权重与特性读取自 configs
            个体表。悬停数值可查看推导过程。</span>
        </div>
        <div class="fs-ov-filters">
          <div class="fs-ov-field">
        ...
        <table class="fs-table fs-ov-table">
          ...
          <tbody>
            <tr v-for="i in pmIndividuals" :key="i.id" class="fs-ov-clickable"
              ...
              :aria-label="`选择个体 ${i.name}`" :aria-pressed="pmIndividual?.id === i.id" @click="pmIndividualId = i.id"
              @keydown.enter.prevent="pmIndividualId = i.id" @keydown.space.prevent="pmIndividualId = i.id">
              <td class="fs-td-strong">{{ i.name }}</td>
              <td>{{ pmCategoryLabel(i.category) }}</td>
              <td>{{ pmWeightText(i) }}</td>
              <td class="fs-cell-dim">{{ pmTraitText(i) }}</td>
            </tr>
            <tr v-if="!pmIndividuals.length">
              <td colspan="4" class="fs-cell-dim">configs 个体表为空</td>
            </tr>
          </tbody>
        </table>
      </div>
      ...
      <div v-if="pmOverview" class="fs-ov-grid">
        ...
        <div class="fs-ov-card fs-ov-main">
          <div class="fs-ov-subject">
            <span class="fs-affix-tag-sm" :class="pmSystem === 'pet' ? 'side-atk' : 'side-def'">{{ pmSystem === 'pet' ?
              '宠物'
              : '坐骑' }}</span>
            <span class="fs-ov-subject-text">{{ pmOverview.individual.name }} · Lv.{{ pmOverview.level }} · {{
              tierLabel(pmTier) }} · 品质 {{ pmOverview.quality }}（{{ pmQualityLabel }}）· 资质 {{ pmOverview.aptitude }} ·
              词条 {{
                pmOverview.activeSlotCount }} 条</span>
          </div>
            ...
            <table class="fs-table fs-ov-table">
              ...
              <tbody>
                <tr v-for="(s, idx) in pmOverview.mainSlots" :key="s.attribute" class="fs-ov-clickable"
                  ...
                  @click="pmPool = `main-${idx}`" @keydown.enter.prevent="pmPool = `main-${idx}`"
                  @keydown.space.prevent="pmPool = `main-${idx}`">
                  <td class="fs-td-strong">{{ s.label }}</td>
                  <td class="fs-td-right fs-cell-num">{{ s.weight }}</td>
                  <td class="fs-td-right fs-cell-num">
                    <CalcBreakdown :value="fmtRange(s.range)" :steps="s.range.calc" :unit="rangeUnit(s.range)" />
                  </td>
                </tr>
              </tbody>
            </table>
          ...
          <div class="fs-ov-section">
            ...
            <div v-else class="fs-ov-pool-gap">当前品质 {{ pmOverview.quality }} 不投放（品质 {{ pmTraitRow?.minQuality ?? 3 }} 起）
            </div>
          </div>
          ...
          <div class="fs-ov-section">
            ...
            <table class="fs-table fs-ov-table">
              <thead>
                <tr>
                  <th>行</th>
                  <th class="fs-td-center">品质门槛</th>
                  <th class="fs-td-center">本品质</th>
                  <th class="fs-td-right">属性数</th>
                </tr>
              </thead>
              <tbody>
                <tr v-for="row in pmExtraRows" :key="row.id" class="fs-ov-clickable"
                  :class="{ 'is-active': pmPool === `row-${row.id}`, 'is-muted': !row.included }" tabindex="0"
                  role="button" :aria-label="`展开 ${row.name} 词条池`" :aria-pressed="pmPool === `row-${row.id}`"
                  @click="pmPool = `row-${row.id}`" @keydown.enter.prevent="pmPool = `row-${row.id}`"
                  @keydown.space.prevent="pmPool = `row-${row.id}`">
                  <td class="fs-td-strong">{{ row.name }}</td>
            ...
            <table class="fs-table fs-ov-table">
              ...
              <tbody>
                <tr v-for="c in pmPoolDetail.candidates" :key="c.attribute" :class="{ 'is-none': c.source === 'none' }">
                  ...
                  <td class="fs-td-center"><span class="fs-ov-src">{{ c.source === 'formula' ? '公式' : c.source ===
                    'curve' ? '曲线' : '缺曲线' }}</span></td>
                </tr>
                <tr v-if="!pmPoolDetail.candidates.length">
                  <td colspan="3" class="fs-cell-dim">该行下无可投放属性</td>
                </tr>
              </tbody>
            </table>
          </template>
          ...
          <table class="fs-table fs-rule">
            ...
            <tbody>
              <tr v-for="slot in fixedSlotOrder" :key="slot">
                ...
                <td>
                  <span v-if="cfg.sub_type_groups[slot]?.sub_types?.length">{{
                    cfg.sub_type_groups[slot].sub_types.map((s) => s.name).join('、')}}</span>
            ...
            <table class="fs-table fs-rule">
              ...
              <thead>
                <tr>
                  <th>子类型</th>
                  <th>部位</th>
                  <th class="fs-th-center">阵营</th>
                  <th>核心属性</th>
                  <th class="fs-th-right">系数</th>
                </tr>
              </thead>
              <tbody>
                <tr v-for="st in allSubTypes" :key="st.id">
                  ...
                  <td class="fs-td-center"><span class="fs-affix-tag-sm"
                      :class="st.side === 'ATK' ? 'side-atk' : 'side-def'">{{ st.side }}</span></td>
                  <td>
          ...
          <div class="fs-rule-wrap">
            <table class="fs-table fs-rule">
              ...
              <thead>
                <tr>
                  <th>阶位</th>
                  <th class="fs-th-center">强化上限</th>
                  <th class="fs-th-right">权重下限</th>
                  <th class="fs-th-right">权重上限</th>
                </tr>
              </thead>
              <tbody>
                <tr v-for="(tw, tier) in cfg.tier_weight" :key="tier">
                  ...
                  <td class="fs-td-right"><input v-model.number="tw.min" type="number" step="0.1" min="0" max="1"
                      class="fs-input fs-exp-num fs-num-right" /></td>
                  <td class="fs-td-right"><input v-model.number="tw.max" type="number" step="0.1" min="0" max="1"
                      class="fs-input fs-exp-num fs-num-right" /></td>
                </tr>
              </tbody>
            </table>
          ...
          <div class="fs-rule-wrap is-scroll">
            <table class="fs-table fs-rule">
              ...
              <thead>
                <tr>
                  <th>属性组</th>
                  <th class="fs-th-center">下限基础</th>
                  <th class="fs-th-center">下限成长</th>
                  <th class="fs-th-center">下限满级</th>
                  <th class="fs-th-center">上限基础</th>
                  <th class="fs-th-center">上限成长</th>
                  <th class="fs-th-center">上限满级</th>
                </tr>
              </thead>
              <tbody>
                <tr v-for="(row, i) in rows" :key="i">
                  <td>{{ attrNames(row.attributes) }}</td>
                  <td class="fs-td-center"><input v-model.number="row.min.base" type="number" step="0.01"
                      class="fs-input fs-exp-num" /></td>
                  <td class="fs-td-center"><input v-model.number="row.min.perLevel" type="number" step="0.01"
                      class="fs-input fs-exp-num" /></td>
                  <td class="fs-td-center"><input v-model.number="row.min.full" type="number" step="0.01"
                      class="fs-input fs-exp-num" /></td>
                  <td class="fs-td-center"><input v-model.number="row.max.base" type="number" step="0.01"
                      class="fs-input fs-exp-num" /></td>
                  <td class="fs-td-center"><input v-model.number="row.max.perLevel" type="number" step="0.01"
                      class="fs-input fs-exp-num" /></td>
                  <td class="fs-td-center"><input v-model.number="row.max.full" type="number" step="0.01"
                      class="fs-input fs-exp-num" /></td>
                </tr>
              </tbody>
            </table>
    ...
    <section v-else-if="activeTab === 'pet_mount'" class="fs-exp-panel" role="tabpanel">
      <div class="fs-exp-block">
        ...
        <div v-for="(rows, sysKey) in cfg.affix_value_curve" :key="sysKey" class="fs-curve-system">
          <div v-if="sysKey === 'pet_mount'" class="fs-block-title" style="margin-top: var(--space-3);">{{
            valueCurveSystemLabel(sysKey) }}</div>
          ...
          <table class="fs-table">
            ...
            <tbody>
              <tr v-for="(group, code) in cfg.attribute_groups" :key="code">
                ...
                <td>{{ group.label }}</td>
                <td><span class="fs-affix-tag-sm" :class="group.side === 'ATK' ? 'side-atk' : 'side-def'">{{ group.side
                }}</span></td>
        ...
        <div v-if="editingGroup" class="fs-group-edit">
          ...
          <div class="fs-edit-attrs">
            ...
            <div class="fs-group-add-attr">
              <TacticalSelect :model-value="addAttrSelection[editingCode] ?? null" size="md" searchable
                placeholder="选择属性…" :options="availableAttrsOptions(editingCode)"
                @update:model-value="(v: string | number | null) => { addAttrSelection[editingCode] = v as string }" />
              <input v-model="addAttrNameSelection[editingCode]" type="text" class="fs-input" placeholder="显示名称"
                style="width:120px" />
              ...
              <tr v-for="(rule, idx) in cfg.forbidden" :key="idx">
                ...
                <td>
                  <div class="fs-forbidden-attrs">
                    <TacticalSelect v-for="(attr, i) in rule.attributes" :key="i" :model-value="attr" size="md"
                      searchable clearable :options="allAttrsOptions"
                ...
                <td><button type="button" class="fs-btn-danger-sm" @click="cfg.forbidden.splice(idx, 1)">删除</button>
                </td>
              </tr>
            </tbody>
          </table>
...
const ALL_ATTRS: AttrInfo[] = [
  ...
  { code: 'hitRate', name: '命中率', isPercentage: true },
  ...
  { code: 'dodgeRate', name: '闪避率', isPercentage: true },
...
/** 部位展示顺序（固定属性表行序；单一来源 EQUIPMENT_SLOTS） */
const fixedSlotOrder = [...EQUIPMENT_SLOTS]
...
/** 品阶中文名（拼音五档标签单一来源：schema.AFFIX_QUALITY_VALUE_LABEL） */
function tierLabel(tier: string): string {
  return AFFIX_QUALITY_VALUE_LABEL[tier as AffixQualityCode] ?? tier
}
...
/* 列对齐 */
.fs-rule thead th.fs-th-center {
  text-align: center;
}

.fs-rule thead th.fs-th-right {
  text-align: right;
}

.fs-rule tbody td.fs-td-center {
  text-align: center;
}

.fs-rule tbody td.fs-td-right {
  text-align: right;
}
...
.fs-ov-table {
  ...
  thead th.fs-td-center {
    text-align: center;
  }

  tbody td.fs-td-right,
  thead th.fs-td-right {
    text-align: right;
  }
}
