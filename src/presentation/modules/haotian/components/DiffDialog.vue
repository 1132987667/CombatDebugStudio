<template>
  <div v-if="open" class="ht-modal" @click.self="close">
    <div class="ht-modal-box ht-diff">
      <div class="ht-modal-hd">
        <span>分支对比 · {{ store.archive?.battleId ?? '—' }} → {{ store.branch?.battleId ?? '—' }}</span>
      ...
      <div class="ht-diff-toolbar">
        ...
        <select v-model="branchKey" class="ht-btn" aria-label="从战斗记录选分支">
          <option value="">从战斗记录选分支…</option>
          <option v-for="r in store.recordings" :key="r.saveKey" :value="r.saveKey">
            {{ r.name }} · {{ r.eventCount }} 事件
          </option>
        </select>
