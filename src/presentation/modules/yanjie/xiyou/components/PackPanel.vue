<template>
  <div class="xy-panel-scroll xy-pack-host">
    <Tabs v-model="sub" :tabs="SUBS" destroy-inactive class="xy-pack-tabs">
      <!-- ── 背包 ── -->
      <template #pack>
                  ...
                  <div v-for="group in displayGroups(c.types)" :key="group.type" class="xy-cabinet-cat">
                    ...
                    <div class="xy-card-grid">
                      <PackItemCard
                        ...
                        @open="openDetail" @use="onUse" @storage="onMoveToStorage" @discard="askCardDiscard" />
                    </div>
                  </div>
                </template>
        ...
        <div class="xy-pack-list xy-panel-tabs">
          ...
          <div class="xy-storage-grid">
            <button v-for="(slot, i) in pack.storage" :key="i" type="button" class="xy-storage-cell"
              :class="{ 'is-empty': !slot.itemId }" @click="openStorageCell(i)">
...
const PACK_CATEGORIES = [
  ...
  { id: 'equip', label: '装备', types: ['武器', '衣服', '饰品'] },
...
/* 固定 tab 头布局：滚动下放到各 tabpanel 内容区（.xy-pack-list），
   一级/二级 tab 头与筛选工具栏保持固定。xy-panel-tabs 类为 feature 态 grid 化豁免。 */
.xy-panel-scroll.xy-pack-host {
  ...
  /* 一级/二级 Tabs 根都撑满高度（二级经 .xy-pack-sub 透传同值） */
  :deep(.tabs-root) {
    flex: 1;
    min-height: 0;
  }
